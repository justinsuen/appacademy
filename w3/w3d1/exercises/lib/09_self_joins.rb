...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT
    DISTINCT start.num, start.company, transfer.name, des.num, des.company
  FROM
    routes start
  JOIN
    routes transfer_des ON (start.company = transfer_des.company
      AND start.num = transfer_des.num)
  JOIN
    stops transfer ON (transfer_des.stop_id = transfer.id)
  JOIN
    routes transfer_line ON (transfer_line.stop_id = transfer.id)
  JOIN
    routes des ON (transfer_line.company = des.company
      AND transfer_line.num = des.num)
  WHERE
    start.stop_id = (
      SELECT
        id
